...
$(document).ready(function(){
    ...
    $("#addAdminSubmit").click(function(e){
        ...
        $.ajax({
            ...
            if(!navigator.onLine){
                $("#fMsg").css('color', 'red').text("Bağlantı hatası! Lütfen internet bağlantınızı kontrol ediniz.");
            }
        });
    });
    ...
    $("#editAdminSubmit").click(function(e){
        ...
        if(formChanges("editAdminForm")){
            ...
            if(!adminId){
                $("#fMsgEdit").text("Admin detayları güncellenirken beklenmedik bir hata meydana geldi.");
                return;
            }
            ...
            $.ajax({
                ...
                if(returnedData.status === 1){
                    ...
                    if(!navigator.onLine){
                        $("#fMsgEdit").css('color', 'red').html("Bağlantı hatası! Lütfen internet bağlantınızı kontrol ediniz.");
                    }
                });
        }
        
        else{
            $("#fMsgEdit").html("Hiç değişiklik yapılmadı.");
        }
    });
            ...
            if(adminId){
                $.ajax({
                    ...
                    if(returnedData.status === 1){
                        ...
                        var newHTML = returnedData._nv === 1 ? "<a class='pointer'>Silme İşlemini Geri Al</a>" : "<i class='fa fa-trash pointer'></i>";
