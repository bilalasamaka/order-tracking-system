...
var appRoot = setAppRoot("myfresh", "myfresh");
...
$(document).ready(function(){
    ...
    $("#loginModalSubmit").click(function(e){
       ...
       if(!email || !password){
           ...
           $("#logInModalFMsg").css('color', 'red').html("Lütfen mail adresinizi ve şifrenizi birlikte giriniz!");
           return;
       }
       
       
       //display progress message
       $("#logInModalFMsg").css('color', 'black').html("Oturum açılıyor. Lütfen bekleyiniz...");
    ...
    $("#selecteddbfile").change(function(e){
        ...
        if(file){
            ...
            $("#dbFileMsg").css('color', 'black').html("Veritabanı aktarılıyor.");
            ...
            $.ajax({
                ...
                if(rd.status === 1){
                    ...
                    $("#dbFileMsg").css('color', 'green').html("Veritabanı aktarımı başarılı.");
...
function vtr_(elem){
    ...
    if(ref){
        ...
        $("#transReceipt").html("<i class='fa fa-spinner faa-spin animated'></i> Makbuz yükleniyor...");
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    $("#transReceipt").html("İşlem bulunamadı.");
                }
            }
        });
    }
    ...
    if(document.hidden || document.onfocusout || window.onpagehide || window.onblur){
        ...
        $.ajax({
            ...
            if(returnedData.status === 0){
                ...
                $("#logInModalFMsg").css('color', 'red').html("Oturumunuz sona erdi. Devam etmek için lütfen giriş yapınız.");
...
function handleLogin(email, password, callback){
    ...
    $.ajax(appRoot+'access/login', {
        ...
        if(returnedData.status === 1){
            ...
            jsonToReturn = {status:0, msg:"Geçersiz e-mail şifre!"};
        }
        ...
        var msg = "Oturum açılamadı. Lütfen internet bağlantınızı kontrol edip, tekrar deneyiniz.";
...
function checkBrowserOnline(changeFlashContent){
    if((!navigator.onLine) && (appRoot.search('localhost') === -1)){
        changeFlashContent ? 
            changeFlashMsgContent('Oturum açılamadı. Lütfen internet bağlantınızı kontrol edip, tekrar deneyiniz.', '', 'red', '', false) 
            : 
            displayFlashMsg('Oturum açılamadı. Lütfen internet bağlantınızı kontrol edip, tekrar deneyiniz.', '', 'red', '', false);
    }
    
    else{
        changeFlashContent ? 
            changeFlashMsgContent('Lütfen tekrar deneyiniz. Hata alıyorsanız iletişime geçiniz.', '', 'red', '', false) 
            : 
            displayFlashMsg('Lütfen tekrar deneyiniz. Hata alıyorsanız iletişime geçiniz.', '', 'red', '', false);
    }
}
